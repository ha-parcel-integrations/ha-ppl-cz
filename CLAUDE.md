...
| change which optional field this carrier populates vs. always returns `None` | Update the affected variant in `const.py`'s **`CAPABILITIES_BY_VARIANT`** in the same commit (not the flat `CAPABILITIES` alias, which the docs site never reads) — it feeds the comparison table on the docs site, so a field that starts (or stops) coming back non-null and isn't reflected there is a wrong claim on the website, not just a stale comment. A variant added here also needs a matching backend under this repo's `auth:` mapping in the docs site's `data/carriers.yml`, or the site claims the wrong way in |
