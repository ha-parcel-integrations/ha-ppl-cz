"""Device triggers for the PPL CZ parcel tracker integration.
...
# Device-trigger type -> bus event fired by the coordinator. Incoming
# (received) parcels get registered/status_changed/delivered; outgoing (sent)
# parcels get status_changed/delivered (no "registered" — same as the rest of
# the suite). No ``*_delivery_time_changed``: PPL CZ's list DTOs expose no ETA.
TRIGGER_EVENTS = {
...
async def async_get_triggers(
    ...
    """Return the list of parcel triggers for a PPL CZ device."""
    return [
        {
